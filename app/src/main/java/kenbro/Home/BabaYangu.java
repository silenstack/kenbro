...
public class BabaYangu extends RecyclerView.Adapter<RecyclerView.ViewHolder> {
    private List<MessaMode> list;
    private Context ctx;

    public BabaYangu(Context context, List<MessaMode> list) {
        this.list = list;
        ctx = context;
    }
    ...
    public void onBindViewHolder(@NonNull RecyclerView.ViewHolder hotel, int i) {
        if (hotel instanceof OriginalViewHolder) {
            ...
            if (get.getStaged().equals("M")) {
                viewHolder.mess.setText(get.getMessage());
