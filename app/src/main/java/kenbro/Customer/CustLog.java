...
public class CustLog extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        ResetPass.setOnClickListener(v -> {
            startActivity(new Intent(getApplicationContext(), CustReset.class));
        });
        ...
        btnLog.setOnClickListener(v -> {
            ...
            if (myUser.isEmpty() || myPass.isEmpty()) {
                ...
                StringRequest stringRequest = new StringRequest(Request.Method.POST, Manage.lCu,
                        response -> {
                            try {
                                ...
                                if (status.equals("1")) {
                                    ...
                                    for (int i = 0; i < dataArray.length(); i++) {
                                        ...
                                        String residence = dataobj.getString("residence");
                                        String username = dataobj.getString("username");
                                        String sstatus = dataobj.getString("status");
                                        String date = dataobj.getString("date");
                                        custSession.loginCust(entry_no, fname, lname, email, phone, residence, username, sstatus, date);
                                    }
