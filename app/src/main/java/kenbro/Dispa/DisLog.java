...
public class DisLog extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        Objects.requireNonNull(getSupportActionBar()).setTitle("Dispatcher Login");
        setContentView(R.layout.activity_dis_log);
        inventSession = new DipSession(getApplicationContext());
        staffModel = inventSession.getUserDetails();
        ...
        ResetPass.setOnClickListener(v -> {
            startActivity(new Intent(getApplicationContext(), DispReset.class));
        });
        CreateAccount.setOnClickListener(v -> {
            startActivity(new Intent(getApplicationContext(), DispReg.class));
        });
        btnLog.setOnClickListener(v -> {
            ...
            if (myUser.isEmpty() || myPass.isEmpty()) {
                ...
                StringRequest stringRequest = new StringRequest(Request.Method.POST, Manage.LDis,
                        response -> {
                            try {
                                ...
                                if (status.equals("1")) {
                                    ...
                                    for (int i = 0; i < dataArray.length(); i++) {
                                        ...
                                        inventSession.loginDisp(serial_no, fname, lname, email, phone, role, username, sstatus, date);
                                    }
