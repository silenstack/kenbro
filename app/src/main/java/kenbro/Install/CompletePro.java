...
import com.example.kenbro.Artisan.NewProj;
...
public class CompletePro extends AppCompatActivity {
    Button dater;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        Objects.requireNonNull(getSupportActionBar()).setTitle("Newly Completed");
        ...
        listView.setOnItemLongClickListener((adapterView, view, i, l) -> {
            ...
            alertDialog.getButton(DialogInterface.BUTTON_POSITIVE).setOnLongClickListener(viewer -> {
                requestQueue = Volley.newRequestQueue(getApplicationContext());
                requestQueue.add(new StringRequest(Request.Method.POST, Manage.wewe,
                        responses -> {
                            try {
    ...
    private void getServ() {
        requestQueue = Volley.newRequestQueue(getApplicationContext());
        requestQueue.add(new StringRequest(Request.Method.POST, Manage.getPro,
                response -> {
                    try {
                        jsonObject = new JSONObject(response);
                        int st = jsonObject.getInt("success");
                        if (st == 1) {
                            ...
                            for (int i = 0; i < jsonArray.length(); i++) {
                                ...
                                servAda = new ProjAda(CompletePro.this, R.layout.acc, servModeArrayList);
    ...
    public boolean onOptionsItemSelected(@NonNull MenuItem item) {
        switch (item.getItemId()) {
            case R.id.Hist:
                startActivity(new Intent(getApplicationContext(), ComplHist.class));
                break;
            default:
        }
