...
if (strlen($_SESSION['userAdmin']) == 0) {
    ...
    if (isset($_POST['approveBtn'])) {
        ...
        $sql = mysqli_query($db, "UPDATE driver set status =1,remarks='' where entry_no = '$customerid '");
        if (!$sql) {
            die("Connection failed: " . $db->connect_error);
        } else {
            echo "<script>alert('Account Activated Successfully')</script>";
            echo "<script>location.href='driver' </script>";
        }
    }
    ...
    if (isset($_POST['rejectBtn'])) {
        ...
        $sql = mysqli_query($db, "UPDATE driver set status =2 ,remarks='$remarks' where entry_no = '$customerid '");
        if (!$sql) {
            die("Connection failed: " . $db->connect_error);
        } else {
                    ...
                    <div class="col-md-12">
                        <div class="card">
                            <div class="card-header">
                                <strong class="card-title">Manage Drivers</strong>
                            </div>
                            <div class="card-body">
                                <table id="bootstrap-data-table-export" class="table table-striped table-bordered">
                                    ...
                                    <tbody>
                                        <?php
                                            $query_run = mysqli_query($db, "SELECT * FROM driver");
                                            $i = 1;
